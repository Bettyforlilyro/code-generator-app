...
- 凡是涉及“状态共享、路由切换、组件复用、数据频繁变化、多人协作维护、复杂表单验证、单页应用（SPA）需求”中两项以上复杂需求，优先选择 VUE_PROJECT
"""
...
**阶段二：执行（可多轮迭代）**
正式开始生成代码，根据用户需求和计划，使用【文件写入工具】依次创建每个文件。

### 通用规则
1）必须通过使用【文件写入工具】依次创建每个文件（而不是直接输出文件代码）
2）进入阶段二执行后，严禁再次进入阶段一规划
3）在执行阶段所有代码文件写入完毕后，需要再次确认一遍没有文件漏写（确保文件数量和文件路径与规划完全一致），如果有漏写，需要及时修复
